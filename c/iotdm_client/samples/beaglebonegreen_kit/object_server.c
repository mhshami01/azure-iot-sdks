...
bool lwm2m_isServerDisabled(void *instance)
{
    server_instance_t *targetP = (server_instance_t *) instance;
    return targetP->disabled;
}

void lwm2m_enableServerInstance(void *instance)
{
    server_instance_t *targetP = (server_instance_t *) instance;
    targetP->disabled = false;
}

static uint8_t prv_get_value(lwm2m_data_t * tlvP,
                             server_instance_t * targetP)
{
    ...
    switch (tlvP->id)
    {
        ...
        //    prop_readIntValue("Server", "Lifetime", &(targetP->lifetime));
            lwm2m_data_encode_int(targetP->lifetime, tlvP);
            if (0 != tlvP->length) return COAP_205_CONTENT;
            else return COAP_500_INTERNAL_SERVER_ERROR;

        case LWM2M_SERVER_MIN_PERIOD_ID:
        //    prop_readIntValue("Server", "Min Period", &(targetP->defaultMinPeriod));
            lwm2m_data_encode_int(targetP->defaultMinPeriod, tlvP);
            if (0 != tlvP->length) return COAP_205_CONTENT;
            else return COAP_500_INTERNAL_SERVER_ERROR;

        case LWM2M_SERVER_MAX_PERIOD_ID:
        //    prop_readIntValue("Server", "Max Period", &(targetP->defaultMaxPeriod));
            lwm2m_data_encode_int(targetP->defaultMaxPeriod, tlvP);
            if (0 != tlvP->length) return COAP_205_CONTENT;
            else return COAP_500_INTERNAL_SERVER_ERROR;
        ...
        //    prop_readIntValue("Server", "Disable Timeout", &(targetP->disableTimeout));
            lwm2m_data_encode_int(targetP->disableTimeout, tlvP);
            if (0 != tlvP->length) return COAP_205_CONTENT;
            else return COAP_500_INTERNAL_SERVER_ERROR;

        case LWM2M_SERVER_STORING_ID:
            lwm2m_data_encode_bool(targetP->storing, tlvP);
            if (0 != tlvP->length) return COAP_205_CONTENT;
            else return COAP_500_INTERNAL_SERVER_ERROR;
                                ...
                                lwm2m_object_t * objectP)
{
    ...
    if (targetP->disabled) return COAP_405_METHOD_NOT_ALLOWED;
    ...
    do
    {
        ...
        case LWM2M_SERVER_LIFETIME_ID:
            result = prv_set_int_value(dataArray + i, &(targetP->lifetime));
        ...
        //    if (result == COAP_204_CHANGED)
        //        prop_writeIntValue("Server", "Min Period", targetP->defaultMinPeriod);
            break;

        case LWM2M_SERVER_MAX_PERIOD_ID:
            result = prv_set_int_value(dataArray + i, &(targetP->defaultMaxPeriod));
        //   if (result == COAP_204_CHANGED)
        //        prop_writeIntValue("Server", "Max Period", targetP->defaultMaxPeriod);
            break;
        ...
        //    if (result == COAP_204_CHANGED)
        //        prop_writeIntValue("Server", "Disable Timeout", targetP->disableTimeout);
            break;
